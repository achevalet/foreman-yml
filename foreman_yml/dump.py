...
class ForemanDump(ForemanBase):
    ...
    # dump functionality
    def dump(self, object=None, search=None, generate_files=False, generate_files_dir=None):
        dumpdata = {}
        all_objects = []
        ...
        if object is not None:
            if object not in supported_objects:
                log.log(log.LOG_ERROR, "Object type must be one of %s" %supported_objects)
                sys.exit(1)
            all_objects = [object]
        else:
        ...
        if generate_files == False:
            ...
            log.log(log.LOG_INFO, "Writing yaml files to " + generate_files_dir)
            self.ensure_dir(generate_files_dir)
            for obj in dumpdata:
                objdir = generate_files_dir + '/' + obj
                log.log(log.LOG_DEBUG, "Creating folder " + objdir)
                ...
                for elmt in dumpdata[obj]:
                    ...
                    log.log(log.LOG_DEBUG, "Writing file " + yml_file)
